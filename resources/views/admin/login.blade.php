...
<title>{{ __('Admin Login') }} &mdash; Voxa Center</title>
...
<style>
:root{--navy:#1e293b;--navy-dk:#0f172a;--cyan:#3b82f6;--mid:#2563eb;--lav:#94a3b8;--lav-sub:#eff6ff;--lav-bdr:#bfdbfe;--border:#e2e8f0;--bg:#f8fafc;--slate:#64748b;--ink:#0f172a;--font:'Plus Jakarta Sans',sans-serif;--mono:'Fira Code',monospace}
*{box-sizing:border-box}
body{font-family:var(--font);background:linear-gradient(135deg,var(--navy-dk) 0%,var(--navy) 50%,#1e40af 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;margin:0;-webkit-font-smoothing:antialiased}
...
.login-brand .brand-icon{width:48px;height:48px;background:var(--cyan);border-radius:12px;display:inline-grid;place-items:center;margin-bottom:12px}
.login-brand h1{font-family:var(--font);font-weight:800;font-size:28px;color:var(--navy);letter-spacing:-.04em;margin:0}
.login-brand p{font-size:14px;color:var(--slate);margin:8px 0 0}
...
.btn-login{width:100%;background:var(--navy);color:#fff;border:none;border-radius:10px;padding:14px;font-size:15px;font-weight:700;font-family:var(--font);cursor:pointer;transition:all .15s;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 16px rgba(30,41,59,.25)}
.btn-login:hover{background:#1e40af;transform:translateY(-1px)}
.check-row{display:flex;align-items:center;gap:8px;margin-bottom:20px}
.check-row input{accent-color:var(--navy)}
...
<div class="login-card">
  ...
  <form action="{{ route('admin.login') }}" method="POST">
    ...
    <div class="fg">
      ...
      <input type="email" name="email" class="fc" placeholder="{{ __('Your email address') }}" value="{{ old('email') }}" required autofocus autocomplete="off">
    </div>
    <div class="fg">
